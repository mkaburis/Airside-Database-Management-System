...
-- Create the database
CREATE DATABASE airsidedb WITH OWNER = airport_admin ENCODING = 'UTF8' TABLESPACE = pg_default CONNECTION LIMIT = -1;
...
CREATE TABLE Users(UserID SERIAL PRIMARY KEY, UserName VARCHAR(20), Password VARCHAR(60), Permission INTEGER);
...
CREATE TABLE Destinations(AirportCode VARCHAR(3) PRIMARY KEY, AirportName TEXT, City TEXT, AdministrativeDivision TEXT, Country TEXT);


CREATE TABLE Routes(RouteID SERIAL PRIMARY KEY, AirlineCode VARCHAR(2) REFERENCES Airlines NOT NULL, FlightNo VARCHAR(10), IsActive BOOLEAN, FliesTo VARCHAR(3) REFERENCES Destinations NOT NULL, FliesFrom VARCHAR(3) REFERENCES Destinations NOT NULL);


CREATE TABLE Passengers(PassengerID SERIAL PRIMARY KEY, FirstName VARCHAR(50), LastName VARCHAR(50), DOB DATE, Nationality VARCHAR(50), DLNo VARCHAR(50) UNIQUE, PassportNo VARCHAR(50) UNIQUE, TSAPre BOOLEAN, HomeAirport VARCHAR(3) REFERENCES Destinations NOT NULL);


CREATE TABLE FlightLogs(FlightID SERIAL PRIMARY KEY, DepartureTime TIMESTAMP(0), ArrivalTime TIMESTAMP(0) NULL, GateNo VARCHAR(3) REFERENCES Concourses NOT NULL, IsDelayed BOOLEAN, AircraftID VARCHAR(3) REFERENCES Planes NOT NULL, RouteID SERIAL REFERENCES Routes NOT NULL);


CREATE TABLE PassengerFlights(FlightID SERIAL REFERENCES FlightLogs(FlightID) NOT NULL, PassengerID SERIAL REFERENCES Passengers(PassengerID) NOT NULL, CheckedIn BOOLEAN, Connecting BOOLEAN);
